# Atualizações automáticas (TrophyDesk)
...
2. Gere o instalador **assinado**:
   ...
   ```
3. Crie um release no GitHub (`Cleber-Sanches/TrophyDesk`) e anexe:
   - `TrophyDesk_X.Y.Z_x64-setup.exe`
   - `TrophyDesk_X.Y.Z_x64-setup.exe.sig`
4. Gere o manifesto:
   ```bash
   node tools/write-latest-json.mjs --version X.Y.Z --url "https://github.com/Cleber-Sanches/TrophyDesk/releases/download/vX.Y.Z/TrophyDesk_X.Y.Z_x64-setup.exe"
   ```
...
```
https://github.com/Cleber-Sanches/TrophyDesk/releases/latest/download/latest.json
```
...
- Privada: `keys/trophydesk.key` (não versionar — está no `.gitignore`)
